...
\label{eq:phi}
\boxed{\ \
\varphi(\lambda)\ =\ -\tfrac12\|\lambda\|^2
\ +\ \min_{\alpha\in D}\ c(\lambda)^{\!\top}\alpha,
\qquad
c_i(\lambda)=-\bigl(1+y_i\inner{\lambda}{\phi_i}\bigr),
\ \ }
\end{equation}
and the Lagrangian dual of~\eqref{eq:split} is $\max_{\lambda\in\Hs}\varphi(\lambda)$.
...
$\varphi=\varphi_{\mathrm{sm}}+\varphi_{\mathrm{pl}}$ where
$\varphi_{\mathrm{sm}}(\lambda)=-\tfrac12\|\lambda\|^2$ is smooth and known in
closed form, and $\varphi_{\mathrm{pl}}(\lambda)=\min_{\alpha\in D}c(\lambda)^{\!\top}\alpha$
is a minimum of finitely many affine functions of $\lambda$ (one per vertex of $D$),
hence concave polyhedral. $\varphi$ fails to be differentiable exactly where
...
the kinks of $\varphi$ sit \emph{exactly} on the support-vector condition
$y_if(x_i)=1$ of~\eqref{eq:kkt}. Since any non-degenerate SVM has free support
vectors, the maximiser of $\varphi$ \emph{is} a point of non-differentiability. This
...
therefore $\max_\lambda\varphi(\lambda)=v(\mathrm{D}')=v(\mathrm{D})$, and the
maximiser is $\lambda^\star=-w^\star$.
\end{proposition}
The multiplier of the relaxed identity is thus, up to sign, the primal weight
vector itself. A supergradient of $\varphi$ at $\lambda$ is the violation of the
relaxed constraint,
\begin{equation}
\label{eq:supergrad}
...
\label{sec:whatitis}
It is worth identifying $\max_\lambda\varphi$ in closed form. Substituting
...
\subsection{Relation to bundle methods for regularised risk minimisation}
\label{sec:bmrm}
Section~\ref{sec:whatitis} showed that $\max_\lambda\varphi$ is the hinge-loss
primal, so it is fair to ask whether (A1) is anything more than the known
cutting-plane / bundle methods applied directly to that primal (Joachims KDD~2006;
Teo et al.\ JMLR~2010; Franc \& Sonnenburg ICML~2008), and in particular whether
dualising the representer identity buys anything at all. We state the relationship
precisely rather than leave it implicit, because the answer is sharper than it
first appears.

Those methods minimise $\tfrac{\lambda}{2}\|w\|^2+\tfrac1m\sum_i\ell(f_i,y_i)$ and
take as their cut the subgradient of the empirical risk, whose support is
$\{i: y_if_i<1\}$. Our oracle instead returns the knapsack solution
$\alpha^\star(\lambda)$. These are not merely similar: measured on \textsc{wine}
with a Gaussian kernel at random multipliers, $\alpha^\star(\lambda)$ agrees with
the plain hinge pattern $[\,C\ \text{if}\ y_if_i<1,\ 0\ \text{otherwise}\,]$ on only
$41\%$--$83\%$ of the samples, but agrees with the \emph{biased} pattern
$[\,C\ \text{if}\ y_i(f_i+b)<1\,]$ on $100\%$ of them, where $b=-\theta$ is the
multiplier the knapsack itself produces. In other words:
\begin{center}
\emph{the oracle of (A1) is the hinge-loss oracle with the bias optimised out
exactly, at every iteration,}
\end{center}
and the sorting of Section~\ref{sec:knapsack} is precisely that one-dimensional
minimisation over $b$.

This is what the dualisation buys, and it is not cosmetic. The cited methods drop
the bias term --- as does the implementation of Tortorella \& Ferrante
(\texttt{bundle-svm}, 2017), whose decision function is $\operatorname{sign}\sum_i
u_i\kappa(x_i,x)$ with no intercept --- and they drop it for a reason: with a bias
the empirical risk is no longer a plain separable sum, and a closed-form
subgradient is no longer available. What restores it is exactly the equality
$y^{\!\top}\alpha=0$, which is in $D$ \emph{only because} $b$ was a free variable of
\eqref{eq:primal}. Relaxing the representer identity, rather than writing the primal
down directly, is what puts that equality in the relaxed problem and hence what
makes the biased oracle closed-form.

Two further things follow from the dual route that the direct primal route does not
give: the master multipliers return a feasible $\hat\alpha\in D$ and therefore the
SVM dual variables themselves (Section~\ref{sec:dw}), and $\varphi(\lambda)$ and the
restricted problem bracket $v(\mathrm{D})$ from both sides, whereas the primal
methods bound only their own objective. We claim no more than this: the family of
cuts is the same, the function being optimised is the same, and what the Lagrangian
formulation adds is the bias, the primal variables and the two-sided bound.
...
\[
\varphi_{\mathrm{pl}}(\lambda)\ \le\ c(\lambda)^{\!\top}\alpha^j
...
\label{sec:dw}
Let $\hat\alpha:=\sum_j\vartheta_j\alpha^j$. Since $\vartheta\in\Delta^J$ and
...
\item $\varphi(\lambda)\le v(\mathrm{D})\le \tfrac12\hat\alpha^{\!\top}Q\hat\alpha-\one^{\!\top}\hat\alpha$
...
\begin{equation}
\label{eq:budgetdual}
\varphi(\alpha)=\sum_{i,k}\alpha^k_i-\tfrac12\sum_{k}(\alpha^k)^{\!\top}Q^k\alpha^k
...
$0\le\alpha\le (C+D)\one$, making $\mathrm{dom}\,\varphi$ compact. We verified
